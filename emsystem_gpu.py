...
class EMSystem:
    ...
    def AddModel(self, model):
        ...
        thread_ct = (16, 16, 4)
        block_ct_x = int(math.ceil(float(self.shape[0]) / thread_ct[0]))
        block_ct_y = int(math.ceil(float(self.shape[1]) / thread_ct[1]))
        block_ct_z = int(math.ceil(float(self.shape[2]) / thread_ct[2]))
        block_ct = (block_ct_x, block_ct_y, block_ct_z)
        model.modeller[block_ct, thread_ct](self.x, self.y, self.z,
    ...
    def Initialize(self):
        ...
        """
        self.state = EMState(cuda.to_device(np.zeros(self.shape)),
                             cuda.to_device(np.zeros(self.shape)),
                             cuda.to_device(np.zeros(self.shape)),
                             cuda.to_device(np.zeros(self.shape)),
                             cuda.to_device(np.zeros(self.shape)),
                             cuda.to_device(np.zeros(self.shape)))
        self.epsilon_x = cuda.to_device(np.full(self.shape, 1/(4e-7*np.pi*299792458**2)))
        self.epsilon_y = cuda.to_device(np.full(self.shape, 1/(4e-7*np.pi*299792458**2)))
        self.epsilon_z = cuda.to_device(np.full(self.shape, 1/(4e-7*np.pi*299792458**2)))
        self.mu_x = cuda.to_device(np.full(self.shape, 4e-7*np.pi))
        self.mu_y = cuda.to_device(np.full(self.shape, 4e-7*np.pi))
        self.mu_z = cuda.to_device(np.full(self.shape, 4e-7*np.pi))
        self.sigma_x = cuda.to_device(np.zeros(self.shape))
        self.sigma_y = cuda.to_device(np.zeros(self.shape))
        self.sigma_z = cuda.to_device(np.zeros(self.shape))
        self.time = [0.]
        models = self.models
        self.models = []
        for model in models:
        ...
        self.states = [EMState(self.state.Ex.copy_to_host(),
        ...
        self.mstate = EMMaterialState(cuda.to_device(np.zeros(self.shape)),
                                      cuda.to_device(np.zeros(self.shape)),
                                      cuda.to_device(np.zeros(self.shape)),
                                      cuda.to_device(np.zeros(self.shape)),
                                      cuda.to_device(np.zeros(self.shape)),
        ...
        block_ct_z = int(math.ceil(float(self.shape[2]) / thread_ct[2]))
        block_ct = (block_ct_x, block_ct_y, block_ct_z)
        MUpdater[block_ct, thread_ct](self.shape, self.state.Ex, self.state.Ey, self.state.Ez,
    ...
    def Reset(self, t=None):
        ...
        self.state = EMState(cuda.to_device(self.states[0].Ex),
                             cuda.to_device(self.states[0].Ey),
                             cuda.to_device(self.states[0].Ez),
                             cuda.to_device(self.states[0].Hx),
                             cuda.to_device(self.states[0].Hy),
                             cuda.to_device(self.states[0].Hz))
    ...
    def UpdateState(self, dt, f=lambda state, *fargs: None, *fargs):
        ...
        block_ct_z = int(math.ceil(float(self.shape[2]) / thread_ct[2]))
        block_ct = (block_ct_x, block_ct_y, block_ct_z)
        Updater[block_ct, thread_ct](dt, self.dx, self.dy, self.dz, self.shape,
